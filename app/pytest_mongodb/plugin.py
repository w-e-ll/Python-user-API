...
def pytest_addoption(parser):
    ...
    parser.addini(
        name='mongodb_engine',
        help='The database engine to use [pymongo]',
        default='pymongo')
...
def pytest_configure(config):
    global _engine
    _engine = config.getoption('mongodb_engine') \
              or config.getini('mongodb_engine')


@pytest.fixture(scope='function')
def mongodb(pytestconfig):
    dbname = pytestconfig.getoption('mongodb_dbname') \
             or pytestconfig.getini('mongodb_dbname')
...
def make_mongo_client(config):
    engine = config.getoption('mongodb_engine') \
        or config.getini('mongodb_engine')
    host = config.getoption('mongodb_host') \
        or config.getini('mongodb_host')
    if engine == 'pymongo':
...
def load_fixtures(db, config):
    basedir = config.getoption('mongodb_fixture_dir') \
              or config.getini('mongodb_fixture_dir')
